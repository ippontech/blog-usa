...
The constructor of the class `HipSlacker` splits the command into multiple ones and initializes the configuration (payload) with a default one.
...
Then the content of the command is iterated and the configuration is updated. The command can contain the application type, the name, the database type and the port. If you're familiar with JHipster, those parameters have no secret for you!

For the application and database type, the bot simply look for keywords and update the configuration. For the port and the name, the bot uses the command after the keyword (ex: `named my-awesome-app` or `port 8080`).
...
The response from the `generate-application` endpoint is an id and we can use this id to get the status of the generation. That way the bot will post the status of the generation on your Slack channel and keep you updated. The bot will post the link of the GitHub repository if the generation is finished or an error if the generation failed.
...
The main idea of the bot was to be user-friendly and not having to specify all the JHipster's configuration in a command. However multiple improvements can be done to the bot to extend its functionalities, here a small list:
...
Creating a Slack bot using Python is pretty easy and by dedicating the application generation to JHipster Online you will avoid a lot of extra work. The main challenge is on how the bot will parse commands and generate a JHipster's configuration from them. That all depends on how the bot will generate the configuration and on how much complex you want a command to be. A flow with questions/answers can be used to give the user more control on the JHipster's configuration. But in this case, why not using directly JHipster Online ?
