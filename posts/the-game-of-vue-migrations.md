...
The guidance for upgrading 2 to 3 previously shared did highlight some [limitations](https://v3.vuejs.org/guide/migration/migration-build.html#known-limitations) and [preparations](https://v3.vuejs.org/guide/migration/migration-build.html#preparations) before [installation of an intermediary build](https://v3.vuejs.org/guide/migration/migration-build.html#installation). For large-scale applications, it might make sense to do it that way. Perhaps for smaller ones, though, another way could be to generate a new Vue 3 project via the Vue CLI then copy over the existing Vue components. If this simpler attempt cannot be completed for whatever reason, at least there is the more complex transition as a backup.

Using the Vue CLI, create the Vue 3 app with the same plugins of Jest and TypeScript just under a different name. In the original project, switch to a new branch. Use the new branch to compare 2 to 3 when copying existing files between versions.

The biggest differences should be noticed in the [`package.json`](https://github.com/matthewreed26/game-of-life/blob/unit-jest-typescript-intentionality-upgrade/package.json), [`main.ts`](https://github.com/matthewreed26/game-of-life/blob/unit-jest-typescript-intentionality-upgrade/src/main.ts), and the replacement of [`Vue.extend`](https://github.com/matthewreed26/game-of-life/blob/unit-jest-typescript-intentionality/src/components/game-of-life/GameOfLife.component.ts#L17) with [`defineComponent`](https://github.com/matthewreed26/game-of-life/blob/unit-jest-typescript-intentionality-upgrade/src/components/game-of-life/GameOfLife.component.ts#L17) in the component files. The replacement makes it easier to test too after modifying the [`shims-vue.d.ts`](https://github.com/matthewreed26/game-of-life/blob/unit-jest-typescript-intentionality-upgrade/src/shims-vue.d.ts) as it then no longer requires [this aforementioned quick fix](https://github.com/vuejs/vue-test-utils/issues/255). With that, the app has been upgraded to Vue 3!

## Keep On Changing

In this blog we explored a transformational journey starting with a primitive Vue 2 app lacking a testing suite or more proper design principles and ending with a Vue 3 application written in TypeScript that can employ Test-Driven Development going forward. Change is hard to wrangle with but it is happening all around us in the software industry. What makes software engineering especially difficult is keeping up with this fast pace of change.

We know Vue 2 will become outdated just as Struts has in front-end programming, Python 2 has made way for Python 3, and various other technologies that have come before now. This blog may not stand the test of time as individuals/companies/teams move on from Vue 2; but, maybe this blog reaches an audience somewhere that needs the information now to take the next steps forward.

Life is our openness to cope with these shifting constructs and being ready for whatever is next. A large part of that mentality is recognizing where the winds of change are blowing from/to. This is the ["Positive Technology" mindset that Ippon](https://us.ippon.tech/) tries to teach all its consultants. If this article helped facilitate your project's migration, [get in contact with us](https://us.ippon.tech/contact/) so we can hear about your success story! Or, if you resonated with the potential you just read about, [let's discuss](https://us.ippon.tech/contact/) how we may further guide you and your company's transformation.
