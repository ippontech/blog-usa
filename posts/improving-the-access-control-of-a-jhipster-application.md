...
- HTTP verb
    - **user** can get an user using the verb `GET`
    - only **admin** can remove an user using the verb `DELETE`

Thereby, the application's API access-control can be configured at an url level or more specifically at a HTTP verb level. That gives some nice flexibility and allow to achieve any access-control scenarios.
...
Above, the default behavior of a JHipster application with all the rules being chained using the method `antMatchers`. The pattern value `**` simply means that it will match any request so `"/api/**"` matches the application's API.
...
Both methods use the url `/api/users/{login}` but with a different HTTP verb (`GET` and `DELETE`). The annotation `@PreAuthorize` makes sure that the user can invoke the method. In the example above, the call to delete an user will fail if the authenticated user is not an **admin**.

The annotation `@PreAuthorize` can be used to have a very granular access-control and solve the problem issued in the introduction.

## Entities owned by an user

One last concept about the backend that might be useful to implement is to have an access-control at the user level. That way a regular user will only have access to its entities and will not be allow to view other users entities. That can be  done at the UI level but that will not protect a malicious user to brute force the API with a range of IDs for example.
...
The attribute `hasAnyAuthorities` defines which authorities are required to let the user uses the corresponding route.

In order to hide UI elements for non-admin user, a new property `isAdmin` can be used like below:
...
Since the authorities are shared with the UI, the same logic can be done with the frontend's routes to avoid having the regular user accessing unauthorized content. And finally with the help of prebuilt JHipster functions, page's elements like link/button can be hided for regular users and showed for admins.
