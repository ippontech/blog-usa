...
In eBPF's case, eBPF development packages are needed for compilation. Once the eBPF program is compiled, it will most likely be loaded into the eBPF kernel subsystem with the help of a user-space application that loads and manages the program. The user-space code will also likely be distributed with the eBPF code.
...
- eBPF
	- The eBPF verifier makes eBPF programming more rigid, but in doing so attempts to detect improper memory access, infinite loops, code-complexity issues such as too large of a program stack (512 byte limit), and everything in between.
...
If you don't know what you're doing when writing a kernel module, you can completely crash your system. However, you _need to know what you're doing_ to get an eBPF program to do any damage.
