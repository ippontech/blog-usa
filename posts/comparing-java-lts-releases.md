...
```
If you want to test something out quickly, you could mock up your application using `jshell`.  Then you can take the commands you painstaikingly tested and copy/paste them into a `poc.java` file.  This can be attached into an email for your co-worker and forwarded to them.  They can then run the application in one line and, assuming you are using native libraries only, the application will run perfectly.  This is truly a bign step for Java; Oracle has worked tirelessly to make Java a language that can stand the test of the modern programming paradigm.  
